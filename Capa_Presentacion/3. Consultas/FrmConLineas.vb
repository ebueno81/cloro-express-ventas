Public Class FrmConLineas
    ...
    Public Sub Cargar_Grid(ByVal Cadena As String)
        Dgv01.DataSource = c_Neg_MnLineas.get_Linea_Datos(Cadena, "DGV")
        With Dgv01
    ...
    Private Sub TxtBus_Art_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtBus_Art.TextChanged
        Call Cargar_Grid(" and c_anula_reg=0 and c_desc_linea like '%" & TxtBus_Art.Text & "%' order by c_desc_linea")
    End Sub
            ...
            If .RowCount > 0 Then
                ...
                If fila > -1 Then
                    ...
                    If Val(TxtVar.Text) = 1 Then
                        FrmRepEnvases.TxtCod_Linea.Text = .Rows(fila).Cells("Codigo").Value
                        FrmRepEnvases.TxtLinea.Text = .Rows(fila).Cells("Descripcion").Value
                    End If
                    'Mostramos salida por ventas...
                    If Val(TxtVar.Text) = 2 Then
                        FrmRptSalAlm.TxtCod_Linea.Text = .Rows(fila).Cells("Codigo").Value
                        FrmRptSalAlm.TxtLinea.Text = .Rows(fila).Cells("Descripcion").Value
                    End If
                    'Mostramos reporte de estado...
                    If Val(TxtVar.Text) = 3 Then
                        FrmRepEnvasesEstado.TxtCod_Linea.Text = .Rows(fila).Cells("Codigo").Value
                        FrmRepEnvasesEstado.TxtLinea.Text = .Rows(fila).Cells("Descripcion").Value
                        FrmRepEnvasesEstado.BtnConFamilia.Focus()
                    End If
                    'Mostramos reporte de estado VALORIZADO...
                    If Val(TxtVar.Text) = 4 Then
                        FrmRptSalAlmValor.TxtCod_Linea.Text = .Rows(fila).Cells("Codigo").Value
                        FrmRptSalAlmValor.TxtLinea.Text = .Rows(fila).Cells("Descripcion").Value
                        FrmRptSalAlmValor.BtnConFamilia.Focus()
                    End If
